...
Publishing `latest` is blocked unless the local gates and `npm run release:check` pass. After publish, run `npm run release:published-check` against the published package spec, for example `ATLAS_WIKI_PUBLISHED_SPEC=atlas-wiki@0.2.2 npm run release:published-check`.
