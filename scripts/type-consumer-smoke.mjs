...
try {
  writeFileSync(join(dir, "package.json"), JSON.stringify({ type: "module", dependencies: { "@mandarange/atlas-wiki": `file:${process.cwd()}` } }, null, 2));
  ...
  writeFileSync(join(dir, "index.ts"), 'import { AtlasWiki, packageInfo } from "@mandarange/atlas-wiki";\nconst name: string = packageInfo.name;\nconst open: typeof AtlasWiki.open = AtlasWiki.open;\nconsole.log(name, typeof open);\n');
